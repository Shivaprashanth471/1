...
const GetWorkExperienceScreen = (props: any) => {
	...
	const getExperienceDetails = useCallback(() => {
		setIsLoading(true);
		ApiFunctions.get(
			ENV.apiUrl + 'hcp/6190089f6e943f8660c7b43f/experience?exp_type=fulltime',
		)
			.then(async resp => {
					...
					{profile.length === 0 && (
						<>
							<BaseViewComponent style={{marginHorizontal: 10}}>
								...
								/>
								<ErrorComponent text={'Please add experience'} />
								{showInState && <>{inState}</>}
...
export default GetWorkExperienceScreen;
